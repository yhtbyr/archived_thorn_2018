...
from keras.layers import Flatten, Dropout, Lambda, GlobalAveragePooling2D, merge, Input, Dense, concatenate
...
def model1(weights_path=None):
    ...
    for attribute in attrs:
        outputs.append(Dense(1,kernel_initializer='glorot_uniform', activation='tanh', name=attribute)(last_layer_output))

    non_negative_attrs = ['Repetition', 'Symmetry', 'score']
    for attribute in non_negative_attrs:
        outputs.append(Dense(1, kernel_initializer='glorot_uniform', activation='sigmoid', name=attribute)(last_layer_output))
...
def model2(weights_path=None):
   ...
   # last_layer_output = Lambda(K.sqrt, output_shape=squared_root_normalization_output_shape)(last_layer_output)
    ...
    merged = concatenate(activation_plus_squared_outputs, axis=1)
    ...
    for attribute in attrs:
        ...
        outputs.append(Dense(1, kernel_initializer='glorot_uniform', activation='tanh', name=attribute)(merged))

    non_negative_attrs = ['Repetition', 'Symmetry', 'score']
    for attribute in non_negative_attrs:
        outputs.append(Dense(1, kernel_initializer='glorot_uniform', activation='sigmoid', name=attribute)(merged))

    model = Model(outputs =outputs, inputs=_input)
    if weights_path:
        model.load_weights(weights_path)
    return model
